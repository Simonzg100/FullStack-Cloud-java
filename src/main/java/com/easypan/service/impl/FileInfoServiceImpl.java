...
public class FileInfoServiceImpl implements FileInfoService {
    ...
    public UploadResultDto uploadFile(SessionWebUserDto webUserDto, String fileId,
                                      MultipartFile file, String fileName, String filePid, String fileMd5,
        ...
        try {
            ...
            //第一个文件,第一次上传，去查询数据库，如果存在的话，直接秒传就好
            if (chunkIndex == 0) {
            ...
            //判断磁盘空间,先放在redis中，获取临时文件大小,有些文件还在上传，此时加入新的文件
    ...
    // 查询数据库 看看是不是存在这个filename的文件在数据库,否则就重新创建
    ...
    public void transferFile(String fileId, SessionWebUserDto webUserDto) {
        ...
        try {
            ...
            if (FileTypeEnums.VIDEO == fileTypeEnum) {
                ...
                //视频生成缩略图 -- 缩略图（Thumbnail）是一种小图像或视频的预览图，
                // 它是原图像或视频的缩小版。
                // 缩略图通常用于在不消耗大量带宽和资源的情况下快速预览大量图片或视频。
            ...
            logger.error("File transcoding failed, File Id:{}, userId:{}", fileId, webUserDto.getUserId(), e);
    ...
    public static void union(String dirPath, String toFilePath,
                             String fileName, boolean delSource) throws BusinessException {
        ...
        try {
            ...
            logger.error("ailed to merge files: {}.", fileName, e);
            throw new BusinessException("An error occurred while merging the file: " + fileName );
        } finally {
            try {
                if (null != writeFile) {
                    writeFile.close();
    ...
    public FileInfo rename(String fileId, String userId, String fileName) {
        ...
        Integer count = this.fileInfoMapper.selectCount(fileInfoQuery);
        if (count > 1) {
            throw new BusinessException("File name " + fileName + " already exists.");
        }
        fileInfo.setFileName(fileName);
        fileInfo.setLastUpdateTime(curDate);
        return fileInfo;
    ...
    private void checkFileName(String filePid, String userId, String fileName, Integer folderType) {
        ...
        Integer count = this.fileInfoMapper.selectCount(fileInfoQuery);
        if (count > 0) {
            throw new BusinessException("A file with the same name already exists in this directory. " +
    ...
    public FileInfo newFolder(String filePid, String userId, String folderName) {
        ...
        Integer count = this.fileInfoMapper.selectCount(fileInfoQuery);
        if (count > 1) {
            throw new BusinessException("Folder " + folderName + " already exists.");
        }
        fileInfo.setFileName(folderName);
        fileInfo.setLastUpdateTime(curDate);
        return fileInfo;
    ...
    public void recoverFileBatch(String userId, String fileIds) {
        ...
        FileInfo fileInfo = new FileInfo();
        fileInfo.setDelFlag(FileDelFlagEnums.USING.getFlag());
        // TODO 这里不应该直接放到跟目录，应该放到原来文件的pid 目录中
        ...
        //将所选文件重命名
        for (FileInfo item : fileInfoList) { // 要recover的文件
            FileInfo rootFileInfo = rootFileMap.get(item.getFileName()); // 目录下的所有文件
            //文件名已经存在，重命名被还原的文件名
            if (rootFileInfo != null) {
                String fileName = StringTools.rename(item.getFileName());
                FileInfo updateInfo = new FileInfo();
