...
public class UserInfoServiceImpl implements UserInfoService {
    ...
    public void register(String email, String nickName, String password, String emailCode) {
        UserInfo userInfo = this.userInfoMapper.selectByEmail(email);
        if (null != userInfo) {
            throw new BusinessException("Email account already exists.");
        }
    ...
    public void resetPwd(String email, String password, String emailCode) {
        UserInfo userInfo = this.userInfoMapper.selectByEmail(email);
        if (null == userInfo) {
            throw new BusinessException("Email account does not exist.");
        }
        //校验邮箱验证码
        emailCodeService.checkCode(email, emailCode);
    ...
    public SessionWebUserDto qqLogin(String code) {
        ...
        if (UserStatusEnum.DISABLE.getStatus().equals(user.getStatus())) {
            throw new BusinessException("Account is disabled and cannot log in.");
        }
        SessionWebUserDto sessionWebUserDto = new SessionWebUserDto();
    ...
    private String getQQAccessToken(String code) {
        ...
        if (tokenResult == null || tokenResult.indexOf(Constants.VIEW_OBJ_RESULT_KEY) != -1) {
            logger.error("Failed to retrieve qqToken:{}", tokenResult);
            throw new BusinessException("Failed to retrieve qqToken");
        }
    ...
    private String getQQOpenId(String accessToken) throws BusinessException {
        ...
        if (tmpJson == null) {
            logger.error("Failed to call QQ API to retrieve openID: tmpJson{}", tmpJson);
            throw new BusinessException("Failed to call QQ API to retrieve openID.");
        }
        ...
        if (jsonData == null || jsonData.containsKey(Constants.VIEW_OBJ_RESULT_KEY)) {
            logger.error("Failed to call QQ API to retrieve openID: {}", jsonData);
            throw new BusinessException("Failed to call QQ API to retrieve openID.");
        }
    ...
    private QQInfoDto getQQUserInfo(String accessToken, String qqOpenId) throws BusinessException {
        ...
        if (StringUtils.isNotBlank(response)) {
            ...
            if (qqInfo.getRet() != 0) {
                ...
                throw new BusinessException("Exception while calling QQ API to retrieve user information.");
            }
            return qqInfo;
        }
        throw new BusinessException("Exception while calling QQ API to retrieve user information.");
