...
public class AccountController extends ABaseController {
    ...
    public ResponseVO sendEmailCode(HttpSession session,
        ...
        try {
            if (!checkCode.equalsIgnoreCase((String) session.getAttribute(Constants.CHECK_CODE_KEY_EMAIL))) {
                throw new BusinessException("The image verification code is incorrect.");
            }
    ...
    private void printNoDefaultImage(HttpServletResponse response) {
        ...
        try {
            ...
            writer.print("Please place the default avatar, default_avatar.jpg, in the avatar directory");
            writer.close();
        } catch (Exception e) {
            logger.error("Output failed without a default image", e);
        } finally {
            writer.close();
        }
    }
    ...
    public ResponseVO updateUserAvatar(HttpSession session, MultipartFile avatar) {
        ...
        try {
            ...
            logger.error("Uploading avatar failed", e);
        }

        // 因为可以qq登录，如果更新了头像，就把qq头像设置为 空
